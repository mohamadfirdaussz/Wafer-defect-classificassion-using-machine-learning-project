...
def plot_model_comparison_bar(summary_df, save_path):
    ...
    csv_path = "model_artifacts/master_model_comparison.csv"

# Load data
    df = pd.read_csv(csv_path)

# Required model order for the chart
    model_order = ["SVM", "LogisticReg", "KNN", "GradBoosting", "XGBoost", "RandomForest", "DecisionTree"]

# Short labels for display (to match your LaTeX figure)
    model_labels = ["SVM", "LR", "KNN", "GBM", "XGB", "RF", "DT"]

# Pivot table to organize F1 scores by Track
    pivot = df.pivot(index="Model", columns="Track", values="Test_F1_Macro")
...
# Reorder models
    ...
    # Save figure
    save_path = "model_artifacts/model_performance_chart.png"
